...
def clean_ocr_text(text: str, digit_ratio_threshold: float = 0.5) -> str:
    """
    ...
    Tokens are kept when ``digit_count/len(word) <= digit_ratio_threshold``.
    """
    if not text:
        return text
    ...
    for word in words:
        ...
        if digit_count / len(word) <= digit_ratio_threshold:
            cleaned.append(word)
    return " ".join(cleaned)
