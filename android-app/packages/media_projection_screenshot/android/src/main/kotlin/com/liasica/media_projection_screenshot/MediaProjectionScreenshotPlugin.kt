...
class MediaProjectionScreenshotPlugin : FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler {
  ...
  // API 34+ requires MediaProjection.registerCallback before createVirtualDisplay.
  private var projectionCallbackRegistered: AtomicBoolean = AtomicBoolean(false)

  private val projectionMainHandler = Handler(Looper.getMainLooper())
  ...
  private fun ensureMediaProjectionCallbackRegistered() {
    val mp = mediaProjection ?: return
    if (!projectionCallbackRegistered.compareAndSet(false, true)) return
    mp.registerCallback(
